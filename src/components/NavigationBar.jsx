    ...
    return (
        <>
            ...
            >
                ...
                />
                <button className="lg:hidden absolute left-16 " onClick={onClick}>
                ...
                <div className="flex justify-between gap-[3vw] w-[50vw] items-center">
                    ...
                    {/* MOBILE 2 */}
                    <div className="flex gap-[2vw]  w-[33vw] lg:w-[7vw] lg:justify-normal justify-between lg:gap-[1.2vw] relative">
                        {InputSearch ? <X className="text-navbar absolute right-[8vw]   top-[0.4vw] z-40 cursor-pointer lg:h-[2.5vw] lg:w-[1.5vw] shrink-0" onClick={() => { setInputSearch(false), fetchDrumData(), setSearch(""), fetchAccessoriesOneData() }} size={25} /> : <SearchIcon className={` cursor-pointer absolute text-navbar lg:h-[2.5vw] lg:w-[1.5vw]  right-[8vw] ${InputSearch ? " z-30 right-[8.6vw] text-bgFah" : " text-navbar transition-all"}`} onClick={handleSearchClick} size={25} />}
                        {InputSearch ? <input type="text" placeholder="Rechecher un produit..." className={`absolute lg:left-[-44vw] left-[-33vw] w-[50vw] h-8 lg:!w-[43.5vw] bg-bgInput lg:!h-[3vw] text-bgFah top-[-1vw] lg:top-[-.2vw] text-[3vw] rounded-lg lg:text-[1vw] Input ${InputSearch ? "opacity-100 z-[999]" : "opacity-0 cursor-default"}`} onChange={(e) => HandleSearch(e.target.value)}  ref={InputFocusRef} value={Search}/> : <input type="text" placeholder="Rechecher un produit..." className={`absolute lg:left-[-44vw] !cursor-default left-[-33vw] w-[50vw] h-8 lg:!w-[43.5vw] bg-bgInput lg:!h-[3vw] text-bgFah top-[-1vw] lg:top-[-.2vw] text-[3vw] rounded-lg lg:text-[1vw] Input ${InputSearch ? "opacity-100" : "opacity-0 cursor-default z-0"}`} onChange={(e) => HandleSearch(e.target.value)} disabled ref={InputFocusRef} value={Search}/>}
                        <Link to={"/Panier"}>
                            ...
                            </div>) : (<ShoppingCartIcon className={`text-navbar absolute lg:h-[2.5vw] lg:w-[1.5vw] cursor-pointer ${InputSearch?"-z-50":""}`} id={path === "/Panier" ? "active" : ""} size={25} />)}
                        </Link>
                        {isDark ? <Sun className={`lg:h-[2.5vw] lg:w-[1.5vw] ${InputSearch?"z-0":"z-[999]"}  text-navbar cursor-pointer`} onClick={LightMode} size={25} /> : <Moon className={`lg:h-[2.5vw] lg:w-[1.5vw]  text-navbar cursor-pointer ${InputSearch?"z-0":"z-[999]"}`} onClick={DarkMode} size={25} />}
