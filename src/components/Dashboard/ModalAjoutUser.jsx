    ...
    return (
        <>
            <div className="w-full h-screen bg-[#0000008e] flex justify-center items-center top-0 left-0 absolute z-100 ">
                <form method="POST" encType="multipart/form-data" className="w-[25vw] h-[28vw] bg-[#ffffff] p-[2vw] flex justify-start flex-col  z-100 rounded-[1vw] relative transition-all" onSubmit={handleSubmit}>
                    <h1 className="text-[#070b2b] text-[1.2vw] text-center underline">Ajouter un utilisateur</h1>
                    ...
                    <input type="text" id="Nom" name="nom" required className="bg-[#070b2b]! p-[1vw]! text-[1vw]" onChange={(e) => setNom(e.target.value)} />
                    <label htmlFor="Prenom">Prénom</label>
                    <input type="text" id="Prenom" name="prenom" required className="bg-[#070b2b]! p-[1vw]! text-[1vw]" onChange={(e) => setPrenom(e.target.value)} />
                    <label htmlFor="Email">Email</label>
                    <input type="email" id="Email" name="email" required className="bg-[#070b2b]! p-[1vw]! text-[1vw]" onChange={(e) => setEmail(e.target.value)} />
                    <div className="ipt1 relative mb-[1vw]!">
                        <label htmlFor="password" id="labelPassword">Mot de passe</label>
                        <input className="bg-[#070b2b]! p-[1vw]! px-[2vw]! text-[1vw]" type={IsLock ? InputType : "password"} name="password" id="password" required
                            onChange={(e) => setMdp(e.target.value)}
                        />
                        <Lock className="absolute top-[2vw] left-1.5 MailIcon text-white" />
                        {On ? (<EyeOff className="absolute top-[2vw] right-[.5vw] text-white cursor-pointer MailIcon" onClick={() => { setIsLock(false), setInputType("password"), setOn(false) }} />) : (<Eye className="absolute top-[2vw] right-[.5vw] text-white cursor-pointer MailIcon" onClick={() => { setIsLock(true), setInputType("text"), setOn(true) }} />)}
                    </div>
