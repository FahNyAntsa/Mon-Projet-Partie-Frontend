    ...
    return (
        <>
            <div className="w-full h-screen bg-[#0000008e] flex justify-center items-center top-0 left-0 fixed lg:absolute z-[99999]">
                <form method="POST" encType="multipart/form-data"  className="lg:!w-[25vw] w-full md:w-[70%] h-auto lg:!h-[32vw] bg-bgFah p-[2vw] flex justify-start flex-col  z-100 rounded-[1vw] gap-[1vw] relative transition-all" onSubmit={handleSubmit}>
                    ...
                    <label htmlFor="Nom" className="text-navbar text-base md:text-xl ">Nom</label>
                    <input type="text" id="Nom" name="nom" required className="bg-navbar md:text-xl rounded-md p-[1vw]! md:h-12 h-8 text-bgFah lg:text-[1vw]" onChange={(e) => setNom(e.target.value)} />
                    <label htmlFor="Prenom" className="text-base md:text-xl text-navbar">Prénom</label>
                    <input type="text" id="Prenom" name="prenom" required className="bg-navbar md:text-xl rounded-md p-[1vw]! md:h-12 text-bgFah h-8 lg:text-[1vw]" onChange={(e) => setPrenom(e.target.value)} />
                    <label htmlFor="Email" className="text-navbar text-base md:text-xl">Email</label>
                    <input type="email" id="Email" name="email" required className="bg-navbar md:text-xl rounded-md p-[1vw]! md:h-12 text-bgFah h-8 lg:text-[1vw]" onChange={(e) => setEmail(e.target.value)} />
                    <div className="ipt1 relative !mb-[1vw]">
                        <label htmlFor="password" id="labelPassword" className="text-navbar text-base md:text-xl ">Mot de passe</label>
                        <input className="bg-navbar md:text-xl rounded-md px-[8vw] md:h-12 text-bgFah h-8 lg:text-[1vw]" type={IsLock ? InputType : "password"} name="password" id="password" required
                            onChange={(e) => setMdp(e.target.value)}
                        />
                        <Lock className="absolute top-[6.5vw] lg:top-[2vw] left-1.5 lg:w-[1.2vw] text-bgFah" size={20}/>
                        {On ? (<EyeOff className="absolute z-[7777]lg:top-[2vw]  lg:right-[.5vw] top-[6.5vw] text-red-500 cursor-pointer lg:w-[1.2vw]" onClick={() => { setIsLock(false), setInputType("password"), setOn(false) }} size={30}/>) : (<Eye className="absolute lg:top-[2vw] left-1 right-[.5vw] text-bgFah cursor-pointer lg:w-[1.2vw]" onClick={() => { setIsLock(true), setInputType("text"), setOn(true) }} size={20} />)}
                    </div>
                    <label htmlFor="Image" className="flex items-center text-navbar text-base md:text-xl justify-center cursor-pointer !mb-[1vw]">
